...
handleSAN(tok,ctx){
 ...
 let base=ctx.baseHistoryLen||0,
     before=ctx.chess.history().length,
     ply=base+before,
     white=ply%2===0,
...
parseComment(text,pos,ctx){
 ...
 for(let p=0;p<parts.length;p++){
   ...
   if(ctx.type==="variation"){
     ...
     // COMMENT IN MAINLINE → DO NOT INTERRUPT NUMBERING
     ctx.container=null;
     ctx.lastWasInterrupt=false;
   }
...
parse(text){
 let main=new Chess(),
     ctx={type:"main",chess:main,container:null,parent:null,lastWasInterrupt:false,prevFen:main.fen(),prevHistoryLen:0,baseHistoryLen:null},
     i=0;
 ...
 while(i<text.length){
   ...
   if(!isSAN){
     ...
     if(EVAL_TOKEN.test(tok) && tok===this.lastLiteralToken)
       continue;
...
class PGNRenderer{static renderAll(r){(r||document).querySelectorAll("pgn").forEach(el=>new PGNGameView(el))}static init(){if(!ensureDeps())return;PGNRenderer.renderAll(document);window.PGNRenderer={run(r){PGNRenderer.renderAll(r||document.body)}}}}
